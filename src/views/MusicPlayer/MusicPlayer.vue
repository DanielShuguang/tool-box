...
<template>
  <div class="flex flex-col md:flex-row h-full">
    ...
    <div
      class="flex-1 border-(1px solid) border-[--borderColor] flex flex-col min-h-0 relative"
      :class="{ 'bg-[--hoverColor]': isDragging }"
      @drop="handleDrop"
      @dragover="handleDragOver"
      @dragleave="handleDragLeave"
    >
      ...
      <div class="flex-1 overflow-auto">
        <div
          ...
          :key="track.id"
          class="flex items-center justify-between px-[12px] py-[10px] hover:bg-[--hoverColor] cursor-pointer transition-all duration-200"
          :class="{ 'bg-[--hoverColor]': index === currentIndex }"
          ...
          <div class="flex items-center gap-[10px] flex-1 min-w-0">
            ...
            >
              <div class="flex items-end gap-[2px] h-[12px]">
                <div class="w-[2px] bg-[--primaryColor] animate-pulse" style="height: 40%"></div>
                <div
                  class="w-[2px] bg-[--primaryColor] animate-pulse"
                  style="height: 80%; animation-delay: 0.1s"
                ></div>
                <div
                  class="w-[2px] bg-[--primaryColor] animate-pulse"
                  style="height: 60%; animation-delay: 0.2s"
                ></div>
              </div>
            </div>
            ...
            <div class="flex-1 min-w-0">
              <div
                ...
                :class="{ 'text-[--primaryColor] font-medium': index === currentIndex }"
              >
                {{ track.title || track.name }}
              </div>
              <div v-if="track.artist" class="truncate text-[12px] text-[--textColor3]">
                {{ track.artist }}
              </div>
            </div>
          </div>
        ...
        <div v-if="playlist.length === 0" class="text-center text-[--textColor3] py-[30px]">
